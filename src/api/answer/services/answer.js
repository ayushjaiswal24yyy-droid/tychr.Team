...
// ─── Essay detection ──────────────────────────────────────────────────────────
// A question is a true essay only if it is long_answer AND has exactly one part.
// Multi-part long_answer questions have each part evaluated as a short answer.

function isEssayQuestion(questionType, partsCount) {
  ...
  async processBulkEvaluation(answerIds) {
    ...
    for (const answerId of answerIds) {
      ...
      if (submission.test_series?.ai_evaluation_enabled === false) {
        strapi.log.info(`Skipping submission ${answerId}: AI evaluation disabled.`);
      ...
      for (const qna of submission.question_n_answer || []) {
        ...
        // True essay = long_answer with a single part (not a multi-part question)
        const isEssay = isEssayQuestion(questionData.question_type, parts.length);

        // 4. Part loop
        for (let i = 0; i < parts.length; i++) {
          const partDef = parts[i];
          const studentPartResponse = studentAnswerJson[`part_${i}`];
          const existingEval = qna.part_evaluations?.find((pe) => pe.part_index === i)
            ?? qna.part_evaluations?.[i]; // fallback for legacy data without part_index
          ...
          if (!canEvaluate) {
            ...
            partAwardedMarks = existingEval ? (existingEval.awarded_marks ?? 0) : 0;
            ...
            try {
              ...
              // Use essay evaluator only for genuine single-part essays.
              ...
              if (aiResult.annotations) {
                qnaAnnotationsByPart.push({ part: i, annotations: aiResult.annotations });
              }
              if (aiResult.improvedAnswer) {
                qnaImprovedAnswersByPart.push({ part: i, improvedAnswer: aiResult.improvedAnswer });
              }
          ...
          // FIX 1: use awarded_marks (schema field name), not marks
          // FIX 2: always set part_index so frontend lookups work reliably
          updatedPartEvaluations.push({
            ...(existingEval ? { id: existingEval.id } : {}),
            part_index: i,
            awarded_marks: partAwardedMarks,
  ...
  async evaluateWithAI(questionText, studentResponse, maxMarks, idealAnswer, commandTerm = null, difficulty = null) {
    ...
    const parsed = await callCF({
      ...
      messages: [
        {
          role: "system",
          content: "You are a strict but fair IB examiner. Grade student responses and provide actionable feedback. Return valid JSON only — no markdown, no explanation.",
        },
        {
          role: "user",
          content: `Grade this IB student response.
...
Return ONLY this JSON:
{
  ...
  "annotations": [
    {
      "quote": "<exact phrase verbatim from the student response>",
      "comment": "<specific issue or praise, max 15 words>"
    }
  ],
  "improvedAnswer": "<rewritten version of the student response that would score full marks>"
}

Annotation rules:
- 2–4 annotations max. Every quote MUST be copied verbatim from the student response.
- Mix positive and corrective annotations.
- Keep comments under 15 words.
- Return [] if nothing useful to annotate.

Improved answer rules:
- Rewrite the student's actual response to score full marks.
- Match the student's approximate length and style.
- Do NOT write a model essay — improve what they wrote.

Confidence: 1.0=clear-cut, 0.7=judgement call, 0.5=borderline, 0.3=very unclear.`,
        },
      ],
    });
    ...
    return {
      ...
      annotations: Array.isArray(parsed.annotations) && parsed.annotations.length > 0
        ? parsed.annotations
        : null,
...
Total marks must not exceed ${maxMarks}.`;
    ...
    const parsed = await callCF({
      ...
      messages: [
        {
          role: "system",
          content: "You are a senior IB examiner specialising in essay assessment. Be specific — name the exact evidence or argument the student used or missed. Return valid JSON only.",
        },
        {
          role: "user",
...
Return ONLY this JSON:
{
 ...
 ${rubric
  ...
  "annotations": [
    {
      "quote": "<exact phrase verbatim from the student essay>",
      "comment": "<specific issue or praise, max 15 words>"
    }
  ],
  "improvedAnswer": "<the student's essay rewritten to score one full band higher>"
}

Annotation rules:
- 3–5 annotations. Every quote MUST be verbatim from the student essay.
- Cover structural, argument/analysis, and evidence comments.
- Keep comments under 15 words.

Improved answer: preserve the student's argument, improve the execution. Keep roughly the same length.

Confidence: 1.0=clearly strong/weak, 0.7=solid judgement, 0.5=borderline band, 0.3=needs human review.`,
        },
      ],
    });
      ...
      if (keys.length > 0) {
        ...
        for (const key of keys) {
          ...
          if (Array.isArray(correctObj[key]?.options)) {
            ...
            const rangeMin = correctObj[key]?.range_min ?? null;
            const rangeMax = correctObj[key]?.range_max ?? null;

            if (!isNaN(studentNum) && rangeMin !== null && rangeMax !== null) {
              isMatch = studentNum >= rangeMin && studentNum <= rangeMax;
              expectedVal = `${rangeMin} – ${rangeMax}`;
            } else {
